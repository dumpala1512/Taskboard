  ...
  if (req.method === "POST") {
      ...
      for (const projectId of projectIds) {
        ...
        if (project) {
          ...
          if (!members.includes(userId)) {
            await projectRepository.update(projectId, { members: [...members, userId] });
            updatedCount++;
          }
        }
      }
    ...
    try {
      ...
      if (taskAction === 'unassign') {
        const tasks = await import("../../../../server/repositories/task.repository").then(m => m.taskRepository.findAll());
        const userTasks = tasks.filter(t => t.projectId === projectId && (t.assigneeId === userId || t.assignees?.includes(userId)));
        
        for (const task of userTasks) {
          await import("../../../../server/repositories/task.repository").then(m => m.taskRepository.update(task.id, {
